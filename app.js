...
/* ── FIREBASE CONFIG ───────────────────────────── */
const FB        = 'https://edu-icfesv5-default-rtdb.firebaseio.com';
const FB_APIKEY = 'AIzaSyAPhvvlfu-qMzEupRqJQyR5bJG_6Zi5xts'
...
const Auth = {
  ...
  _setSession(u)     { try { localStorage.setItem(this.SK, JSON.stringify(u)); } catch {} },
  _getSession()      { try { const s=localStorage.getItem(this.SK); return s?JSON.parse(s):null; } catch { return null; } },
 
  me() {
    if (this._cache) return this._cache;
    // Restaurar sesión desde localStorage al navegar entre páginas
    try {
      const stored = localStorage.getItem(this.SK);
      if (stored) {
        const u = JSON.parse(stored);
        if (u && u.uid) { this._cache = u; return u; }
      }
    } catch {}
    return null;
  },
  ...
  async getAllGlobal() {
    try {
      // /progress tiene el progreso completo con uid, email, level, etc.
      const r    = await fetch(`${FB}/progress.json`);
      const data = await r.json();
      if (!data || typeof data !== 'object') return [];
      // Filtrar entradas vacías o inválidas
      return Object.values(data)
        .filter(u => u && typeof u === 'object' && u.uid)
        .map(u => ({
          uid:         u.uid,
  ...
  updateUser(u) {
    // Actualizar cache en memoria Y localStorage
    this._cache = u;
    this._setSession(u);
  ...
  // Guardar foto de perfil en localStorage (por uid)
  saveAvatar(file) {
    const u = this.me();
    if (!u) return Promise.resolve({ ok:false, error:'No hay sesión activa.' });
    if (file.size > 3 * 1024 * 1024) return Promise.resolve({ ok:false, error:'La imagen debe pesar menos de 3 MB.' });
    return new Promise((resolve) => {
      const reader = new FileReader();
      reader.onload = e => {
        const base64 = e.target.result;
        localStorage.setItem('ei_avatar_' + u.uid, base64);
        resolve({ ok:true, base64 });
      };
      reader.onerror = () => resolve({ ok:false, error:'Error leyendo archivo.' });
...
const Progress = {
  ...
  record(u, correct, subject, qId) {
    ...
    if (correct) {
      ...
      // Acumular XP de mascota por pregunta correcta
      Pet.addPregunta(u);
    } else {
      u.streak = 0;
    }
...
══════════════════════════════════════════════════ */
const PET_STAGES = [
  { stage:0, name:'Huevo',   emoji:'🥚', desc:'Responde preguntas para que eclosione',   days:0,  pregs:0   },
  { stage:1, name:'Cría',    emoji:'🐣', desc:'¡Eclosionó! Sigue respondiendo',           days:3,  pregs:30  },
  { stage:2, name:'Joven',   emoji:'🐥', desc:'Va creciendo — ¡casi adulto!',             days:7,  pregs:80  },
  { stage:3, name:'Adulto',  emoji:'🦅', desc:'¡Tu mascota está en su máximo esplendor!', days:14, pregs:200 },
];
 
// Cuántas preguntas equivalen a 1 día de racha (acelerador)
const PET_PREGS_PER_DAY = 15;
 
const Pet = {
  // Actualiza racha de días + progreso por preguntas
  update(u) {
    u.pet = u.pet || { dayStreak:0, lastDate:'', stage:0, totalDays:0, xp:0 };
    ...
    if (last !== today) {
      ...
      if (last === yesterday) {
        ...
        // Racha rota — reinicia días pero mantiene XP de preguntas
        u.pet.dayStreak = 1;
    ...
    // Calcular etapa combinando días + XP de preguntas
    u.pet.stage = Pet._calcStage(u.pet);
 
    Auth.updateUser(u);
    return u.pet;
  },
 
  // Llamar cada vez que el usuario responde una pregunta correctamente
  addPregunta(u) {
    u.pet = u.pet || { dayStreak:0, lastDate:'', stage:0, totalDays:0, xp:0 };
    u.pet.xp = (u.pet.xp || 0) + 1;
 
    // Bonus de día: cada PET_PREGS_PER_DAY preguntas equivale a haber jugado 1 día extra
    const bonusDays = Math.floor(u.pet.xp / PET_PREGS_PER_DAY);
    u.pet.bonusDays = bonusDays;
 
    const prevStage = u.pet.stage;
    u.pet.stage = Pet._calcStage(u.pet);
    const evolved = u.pet.stage > prevStage;
 
    Auth.updateUser(u);
    return { evolved, stage: u.pet.stage, xp: u.pet.xp };
  },
 
  // Calcular etapa según días reales + días bonus de preguntas
  _calcStage(pet) {
    const effectiveDays = (pet.dayStreak || 0) + (pet.bonusDays || 0);
    const stg = [...PET_STAGES].reverse().find(s => effectiveDays >= s.days);
    return stg ? stg.stage : 0;
  },
 
  // Progreso hacia siguiente etapa (0-100%)
  getProgress(pet) {
    const effectiveDays = (pet.dayStreak || 0) + (pet.bonusDays || 0);
    const cur  = PET_STAGES[pet.stage];
    const next = PET_STAGES[pet.stage + 1];
    if (!next) return 100;
    const range = next.days - cur.days;
    const done  = effectiveDays - cur.days;
  ...
  getStatus(pet) {
    ...
    const xp     = pet.xp || 0;
    if (streak === 0 && xp === 0) return 'Necesita atención 😔';
    if (xp < 15)   return `${xp} preg. respondidas — ¡sigue así! 🌱`;
    if (xp < 50)   return `${xp} preg. — está creciendo 🌟`;
    if (xp < 120)  return `${xp} preg. — ¡gran constancia! 💪`;
    return `${xp} preg. respondidas — ¡Leyenda! 🔥`;
  },
};
...
function requireAuth() {
  ...
  // Refrescar cache en memoria desde localStorage (por si se limpió)
  if (!Auth._cache) Auth._cache = u;
  checkBan(u.username);
